...
interface AgentPerformanceManagerProps {
  agentWalletAddress: string;
}

export default function AgentPerformanceManager({ agentWalletAddress }: AgentPerformanceManagerProps) {
  const { wallet } = useAppStore();
  const [loading, setLoading] = useState(false);
  const [isVerifier, setIsVerifier] = useState(false);
  const [checkingAuth, setCheckingAuth] = useState(false);
  
  // Agent Performance Form - pre-filled with agent wallet address
  const [pnlBucket, setPnlBucket] = useState('0');
  
  // Agent Risk Query - pre-filled with agent wallet address
  ...
  const handleApplyAgentPerformance = async () => {
    if (!wallet.address) {
      setTxStatus({ type: 'error', message: 'Please connect your wallet' });
      return;
    }
    ...
    try {
      ...
      const agentIdBytes32 = ethers.id(agentWalletAddress);
  ...
  const handleQueryAgentRisk = async () => {
    ...
    try {
      ...
      const agentIdBytes32 = ethers.id(agentWalletAddress);
      const risk = await getAgentRisk(wallet.address, agentIdBytes32);
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-500">Please connect your wallet to manage agent performance</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">Manage performance tracking for this agent</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-2">PnL Bucket (-5 to 5)</label>
              <select
                value={pnlBucket}
                onChange={(e) => setPnlBucket(e.target.value)}
                className="w-full px-4 py-3 bg-gray-800/50 border border-gray-700 rounded-xl focus:outline-none focus:border-blue-500 transition-colors"
              >
        ...
        >
          ...
          <div className="space-y-4">
            ...
            {agentRiskScore !== null && (
              ...
              >
                <p className="text-sm text-gray-400 mb-1">Agent Risk Score</p>
      ...
      >
        <div className="flex gap-3">
          ...
          <div className="text-sm text-gray-300">
            ...
            <p>Only authorized verifiers can apply agent performance metrics. Contact the contract owner to get authorized.</p>
          </div>
        </div>
      </motion.div>
    </div>
